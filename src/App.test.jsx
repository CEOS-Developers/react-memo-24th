...
test('태그와 검색어를 함께 적용하고 초기화한다', async () => {
  ...
  expect(screen.getByRole('status').textContent).toContain('검색 결과가 없습니다');
...
test('고정한 메모는 별도 목록으로 이동하고 해제된다', async () => {
  ...
  expect(within(screen.getByRole('region', { name: '고정된 메모' })).getAllByRole('listitem')).toHaveLength(1);
...
test('선택한 메모의 상세 창을 열고 닫는다', async () => {
  ...
  expect(within(screen.getByRole('dialog')).getByRole('heading').textContent).toBe('오늘의 기록');
...
test('메모가 없는 안내는 검색 실패 안내와 구분한다', () => {
  ...
  expect(screen.getByRole('status').textContent).toContain('새로운 메모를 작성해보세요!');
